...
class NATransformerModel(FairseqNATModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--src-embedding-attn", action="store_true",
                            help="copy encoder word embeddings as the initial input of the decoder")
    @classmethod
    ...
    def initialize_output_tokens(self, encoder_out, src_tokens):
        ...
        length_tgt = (length_tgt*1.0).round().long()
...
class NATransformerDecoder(FairseqNATDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.src_embedding_attn = getattr(args, "src_embedding_attn", False)
        self.embed_length = Embedding(256, self.encoder_embed_dim, None)
        if self.src_embedding_attn:
    ...
    def forward(self, normalize, encoder_out, prev_output_tokens, step=0, **unused):
        features, _ = self.extract_features(
            prev_output_tokens,
            encoder_out=encoder_out,
            embedding_copy=(step == 0) & (self.src_embedding_copy or self.src_embedding_attn),
        )
    ...
    def extract_features(
        ...
        **unused
        ...
        if embedding_copy:
            ...
            elif self.src_embedding_attn:
                positions = self.embed_positions(prev_output_tokens).transpose(0, 1)
                src_mask = encoder_out.encoder_padding_mask
                src_encoder_out = encoder_out.encoder_out
                states, attn = self.pos_attn(
                    query=positions,
                    key=src_encoder_out,
